...
case class HttpCrawlerClient(
  ...
  /**
   * Retrieves the [[crawlercommons.robots.BaseRobotRules]] from this [[sprawler.http.client.HttpCrawlerClient]]'s domain.
...
trait Throttler {
   ...
   * Delay rate is determined by [[sprawler.http.client.Throttler]].crawlDelayRate
   */
 ...
 */
private[sprawler] case class PromiseRequest(p: Promise[Boolean])